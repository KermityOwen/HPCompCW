...
int main(){
  ...
  {
    #ifdef _OPENMP
      printf("USING OPENMP. THREAD COUNT %d\n", omp_get_num_threads());
    #else
  ...
  // CANNOT BE PARALLEL - Time steps has to be ran sequentially as time progresses linearly forward.
  // You cannot process the 2nd second if you don't know the state of the 1st second. So on, so forth.
